...
Wall transitions remain nested except for a small clearance-sized organic swell. A `roof_shoulder` may extend outward only when its maximum local expansion is below both `max_overhang_xy` (default 8 mm) and `max_overhang_ratio` (default 8%), with the existing local-slope and solid checks retained. The default `base_thickness = 0` keeps the habitat naturally open at the bottom. Set a positive **Base plate** thickness in the Shape inspector when an importer, resin workflow, or print profile requires a closed floor; the cavity then begins above that plate while the roof and front entrance remain unchanged.
